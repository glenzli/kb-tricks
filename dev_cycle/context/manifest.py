...
def task_matches(task: ManifestTask, pattern: str) -> bool:
    ...
    if any(needle == normalize_text(tag) for tag in task.tags):
        return True
    if task.context and path_matches(pattern, task.context):
        return True
...
def select_tasks(
    ...
    if legacy_tasks:
        warnings.append(
            "legacy manifest format detected; run dev-cycle context migrate-plan --repo <repo> --write"
        )
