...
SPECIAL_FILES = {
    "configChanged": ".dev-cycle/context/config.yaml",
    "manifestChanged": "CONTEXT_PLAN.md",
    "glossaryChanged": ".dev-cycle/context/GLOSSARY.md",
}
...
def fingerprint_reasons(repo: Path, task: ManifestTask) -> list[str]:
    ...
    if not frontmatter:
        return ["context missing frontmatter"]
    fingerprints = frontmatter.get("fingerprint")
    if not isinstance(fingerprints, list) or not fingerprints:
        return ["context missing fingerprint"]
...
def task_action(task: dict[str, Any], repo: Path, draft: bool) -> str:
    ...
    if context and not (repo / context).exists():
        base = "create"
    elif status == "planned":
        base = "create"
...
def draft_target(task_id: str) -> str:
    return f".dev-cycle/context/_draft/{task_id}.md"
...
def plan_task_action(
    ...
    result = {
        ...
        "context": manifest_task.context,
        "targetContext": manifest_task.context,
...
def build_update_plan(
    ...
    if not config_present:
        setup_candidate_files = {
            ...
            if path.startswith(".dev-cycle/context/")
        }
...
def print_markdown(data: dict[str, Any]) -> None:
    ...
    print(f"- New Context candidates: {len(data['newContextCandidates'])}")
    ...
    if data["actions"]:
        ...
        for action in data["actions"]:
            ...
            for reason in action["blockedReasons"]:
                print(f"  - {reason}")
    if data["newContextCandidates"]:
        print()
        print("## New Context Candidates")
        for candidate in data["newContextCandidates"]:
