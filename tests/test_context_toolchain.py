...
FINGERPRINT = PROJECT_ROOT / "tools" / "context_fingerprint.py"
MANIFEST = PROJECT_ROOT / "tools" / "context_manifest.py"
...
def write_context_artifacts(repo: Path, fingerprint: dict) -> None:
...
- **Context-only Answer**: The release module owns package name normalization.
- **Citations**: `.dev-cycle/context/release/packaging.md`
- **Result**: pass

### Q2 Boundary

- **Question**: What happens for empty names?
- **Context-only Answer**: Empty names are rejected before an identifier is returned.
- **Citations**: `.dev-cycle/context/release/packaging.md`
- **Result**: pass
...
""",
            ...
            self.assertTrue((repo / ".dev-cycle" / "context" / "_validation").is_dir())
            ...
            write_context_artifacts(repo, fingerprint)
            ...
            self.assertTrue(documents[".dev-cycle/context/release/packaging.md"]["fresh"])

            index = json.loads(
                (repo / ".dev-cycle" / "context" / "index.json").read_text(encoding="utf-8")
            )
