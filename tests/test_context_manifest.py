...
TOOL = PROJECT_ROOT / "tools" / "context_manifest.py"
...
- [stale] cli-config
  - **ID**: `cli-config`
  - **Context**: `.dev-cycle/context/cli/config.md`
...
- [merged-into-docs] onboarding
  - **ID**: `onboarding`
  - **Context**: `.dev-cycle/context/dev/onboarding.md`
...
class KbManifestTests(unittest.TestCase):
    ...
    def test_only_matches_id_tag_context_and_source_paths(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp) / "project"
            write_manifest(repo)
            ...
            _, by_kb = manifest_json(repo, "--only", ".dev-cycle/context/api/auth-flow.md")
