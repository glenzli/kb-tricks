...
from test_context_audit import PROJECT_ROOT, materialize_fixture, run, sha256

TOOL = PROJECT_ROOT / "tools" / "context_fingerprint.py"
...
class KbFingerprintTests(unittest.TestCase):
    ...
    def test_check_valid_context_doc_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo = materialize_fixture(tmp, "valid-context")
            proc, data = fingerprint_json(
                repo, "--check", ".dev-cycle/context/release/packaging.md"
            )
            self.assertEqual(proc.returncode, 0, proc.stdout + proc.stderr)
            self.assertEqual(data["errors"], [])
    ...
    def test_check_stale_context_doc_fails(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo = materialize_fixture(tmp, "valid-context")
            source = repo / "src" / "release.py"
            source.write_text(source.read_text(encoding="utf-8") + "\n# changed\n", encoding="utf-8")
    ...
    def test_check_dirty_context_doc_fails_without_override(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo = materialize_fixture(tmp, "valid-context")
            source = repo / "src" / "release.py"
            source.write_text(source.read_text(encoding="utf-8") + "\n# dirty\n", encoding="utf-8")
