...
TOOL = PROJECT_ROOT / "tools" / "context_impact.py"
...
def write_impact_repo(repo: Path) -> None:
    (repo / ".dev-cycle" / "context" / "release").mkdir(parents=True)
    (repo / "src").mkdir()
    (repo / "docs").mkdir()
...
""",
    ...
    (repo / ".dev-cycle" / "context" / "release" / "packaging.md").write_text(
        """---
id: "release-packaging"
...
class KbImpactTests(unittest.TestCase):
    ...
    def test_files_maps_context_path_to_task(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp) / "project"
            repo.mkdir()
            write_impact_repo(repo)

            proc, data = impact_json(repo, "--files", ".dev-cycle/context/release/packaging.md")
            self.assertEqual(proc.returncode, 0, proc.stdout + proc.stderr)
            self.assertEqual(data["scopeMode"], "files")
            self.assertEqual(
                data["scope"],
                {"mode": "files", "files": [".dev-cycle/context/release/packaging.md"]},
            )
            self.assertEqual(data["impactedTasks"][0]["id"], "release-packaging")
            self.assertEqual(
                data["impactedTasks"][0]["matchedFiles"],
                [{"file": ".dev-cycle/context/release/packaging.md", "reason": "context"}],
            )
                ...
                [
                    {
                        ...
                        "recommendation": (
                            "add docs.existing or releaseExcluded in .dev-cycle/context/config.yaml"
                        ),
                    }
                ],
            )
