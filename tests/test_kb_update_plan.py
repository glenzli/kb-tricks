...
from test_kb_impact import init_repo, write_impact_repo

TOOL = PROJECT_ROOT / "tools" / "kb_update_plan.py"
...
class KbUpdatePlanTests(unittest.TestCase):
    ...
    def test_unmatched_included_source_becomes_blocked_new_kb_candidate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(data["newKbCandidates"][0]["file"], "src/new_module.py")
            self.assertEqual(data["newKbCandidates"][0]["action"], "create")
            self.assertFalse(data["newKbCandidates"][0]["allowed"])
            self.assertEqual(
                data["newKbCandidates"][0]["blockedReasons"],
    ...
    def test_docs_and_special_changes_are_review_actions(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(data["specialActions"][0]["file"], "KB_PLAN.md")
