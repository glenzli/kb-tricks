...
2. 如果当前环境可访问 `dev-cycle/tools/kb_scaffold.py`，先执行或建议执行 `python3 tools/kb_scaffold.py --repo <target> --dry-run`，展示将创建的 `.agent/kb/config.yaml`、`KB_PLAN.md` 和保留目录。只有在用户确认初始化空白结构时，才执行非 dry-run；不要用 `--force` 覆盖已有文件，除非用户明确要求。
