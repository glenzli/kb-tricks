...
authoritative. KB is routing and context. A review skill may use KB only when
the relevant KB documents are fresh and authoritative.

## Freshness Gate

Before using KB evidence, review skills must check relevant KB frontmatter:
...
If any relevant KB document is stale, dirty, draft-only, untracked, or
`notAuthoritative: true`, skip the KB-specific review layer and report the
...
4. Add the KB cross-check layer only after the freshness gate passes.
...
- Source type: source/config/test/docs/KB/inference.
