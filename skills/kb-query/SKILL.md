...
- **推断必须隔离**：任何没有直接 KB、源码或 docs 支撑的内容，只能放在“不确定性与推断”区块，使用 `⚠️ [推断]` 显式标记，不能混入事实回答。
...
回答必须包含以下结构。优先以 `templates/query-answer.md` 为草稿模板：

```markdown
## 回答 (Answer)
<对用户问题的直接回答。每个事实句附近必须带来源类型标记，例如 [KB]、[源码回退]、[现有 docs]。本区不得出现 [推断]。>

## 不确定性与推断 (Uncertainty & Inference)
<仅放无法由 KB / 源码 / docs 直接证明的判断。没有推断时写“无”。如果有推断，使用 `⚠️ [推断]`。>

## 引用出处 (Citations)
- KB: <引用的 KB 文档路径及相关段落>
- 源码: <如有源码回退，列出文件路径和行号>
- 现有 docs: <如有 docs 回退，列出路径和段落>
- 推断: <如有推断，说明推断依据和不确定性；否则写“无”>

## 知识库状态 (KB Status)
- ✅ 新鲜 / ⚠️ 部分过期 / ⚠️ dirty 或 draft / ❌ 未覆盖
```

### 第 5 步：来源契约 lint (Provenance Lint Gate)
在交付回答前，必须按 `tools/kb_query_lint.py` 的规则自检：

1. `回答 (Answer)` 中每个事实内容行都必须包含 `[KB]`、`[源码回退]` 或 `[现有 docs]`。
2. `[推断]` 不得出现在 `回答 (Answer)` 中，只能出现在 `不确定性与推断 (Uncertainty & Inference)`。
3. 非空推断区必须显式包含 `⚠️ [推断]` 或 `[Inference]`。
4. `引用出处 (Citations)` 必须覆盖回答中实际使用过的来源类型。
5. 如果 released CLI 可用，将草稿写入临时文件或通过 stdin 调用：
   ```bash
   kb query-lint --json answer.md
   # 或
   python3 tools/kb_query_lint.py --json answer.md
   ```
   lint 不通过时，先修正回答结构和来源标注，再输出给用户。

### 第 6 步：盲区上报 (Blindspot Reporting)
